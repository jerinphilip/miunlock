...
class Auth:
    ...
    def login(self, data):
        ...
        try:
            ...
            raise XiaomiError(
                "invalid json but valid &&& start, probably internal error or changed format",
                2,
            )
        logging.debug(data)
        if data["code"] != 0:
            if data["code"] == 70016:
                ...
                raise XiaomiError(
                    "Account server gave unknown code {}, chinese desc is {}".format(
                        data["code"], data["desc"]
                    ),
                    4,
                )
        elif "notificationUrl" in data and data["notificationUrl"].startswith(
        ...
        if response.status_code == 401:
            raise UserError(
                "Sign in failed,don't reuse the same output,use a fresh one", 4
            )
...
class UnlockRequest:
    ...
    def get_params(self, sep):
        logging.debug(
            ...
            + sep
            + b"&".join([k + b"=" + v for k, v in self.params.items()])
        )
        return (
            self.method.encode("utf-8")
            + sep
    ...
    def add_sign(self):
        self.params[b"sign"] = binascii.hexlify(
            hmac.digest(self.DEFAULT_KEY, self.get_params(b"\n"), "sha1")
        )

    def _encrypt(self, value):
        return b64encode(
            AES.new(b64decode(self.auth.ssecurity), AES.MODE_CBC, iv=self.IV).encrypt(
                pad(value)
            )
        )
    ...
    def add_signature(self):
        self.params[b"signature"] = b64encode(
            hashlib.sha1(
                self.get_params(b"&") + b"&" + self.auth.ssecurity.encode("utf-8")
            ).digest()
        )
    ...
    def run(self):
        ...
        if data.get("code", 0) != 0:
            ...
            raise XiaomiError(
                STRINGS["en"]
                .get(data.get("code", -1), STRINGS["en"][-1])
                .format(**data),
                data.get("code", 6),
            )
    ...
    def send(self):
        response = requests.request(
            self.method,
            Url(scheme="https", host=self.host, path=self.path).url,
            data=self.params,
